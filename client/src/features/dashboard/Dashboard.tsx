                ...
                <tr key={u._id} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      {u.kycImageUrl ? (
                        <button onClick={() => setPreviewMedia({ type: 'image', url: u.kycImageUrl as string })} className="bg-slate-700/60 hover:bg-slate-600/80 border border-slate-600 text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-all shadow-sm">View Photo</button>
                      ) : ( 
                        <span className="text-slate-500 text-xs py-1.5 font-medium px-1">No Photo</span>
                      )}
                      {u.kycVideoUrl ? (
                        <button onClick={() => setPreviewMedia({ type: 'video', url: u.kycVideoUrl as string })} className="bg-indigo-600/20 hover:bg-indigo-600/40 border border-indigo-500/30 text-indigo-400 px-3 py-1.5 rounded-lg text-xs font-medium transition-all shadow-sm">View Video</button>
                      ) : ( 
