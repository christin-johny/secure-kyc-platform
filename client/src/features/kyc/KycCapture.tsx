...
const KycCapture = () => {
  ...
  const [kycData, setKycData] = useState<{ loading: boolean; data: any }>({ loading: true, data: null });
  ...
  useEffect(() => {
    const fetchMe = async () => {
      try {
        ...
        const user = res.data;
        if (user && (user.kycImageUrl || user.kycVideoUrl)) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'image') {
        const imageBlob = dataURLtoBlob(capturedImage as string);
        formData.append('image', imageBlob, 'kyc-photo.jpg');
      }
      if (mode === 'video') {
        formData.append('video', recordedVideoBlob as Blob, 'kyc-video.webm');
      }
